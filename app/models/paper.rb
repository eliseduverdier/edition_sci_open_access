class Paper < ApplicationRecord
  ...
  def all_reviews_done?
    ...
    total = reviews.count
    total >= 2 && reviews.where(progression: 'done').count == total
  end
  ...
  def get_reviews_score
    (self.get_reviews.average(:status) * 100) / 3
  end
  ...
  def current_reviews_round_done
    ...
    self.update(status: 1) if self.all_reviews_done?
  end
  ...
  def have_problem_in_review
    (status == 2 || status == 3) && (reviews_count < 1 || need_review)
  end
